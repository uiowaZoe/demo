...
export class SalesAddComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // .pipe(
    //   startWith(''),
    //   map(val => this._filterWholesaler(val))
    // );
  }
  ...
  //     this.wholesalerList = wholesalerList;
  //   });
  //   return value ? this.wholesalerList.slice() : this.wholesalerList.filter(item => item.name.toLowerCase().indexOf(filterValue) === 0);
  // }
  ...
  changeCurrency(type) {
    if (type === 'base') {
      if (this.baseCurrency === '$') {
        ...
        this.baseCurrency = '$';
        this.baseLocation = 'USA';
      }
    } else {
      if (this.sellCurrency === '$') {
        this.sellCurrency = '¥';
        this.sellLocation = 'CNA';
      } else {
        this.sellCurrency = '$';
